...
- First off, we can use `curl_translate` to get example query for `httr` from ECHO API documentation
...
- Then we take result from `curl_translate` to generate a simple non-spatial request 
- We'll come back to this in following slides and see pulling in ESRI REST service for FRS data
...
- The `tidycensus` and `tigris` packages are excellent R packages that wrap the US Census API
- Just a few examples of their use here
...
- Here we'll just query a USA Waterbodies ESRI REST service layers using `httr`
- Note we have to figure out how to parameterize our request body - *this can be a huge pain!!*
...
- Here's we'll subset to Oregon just as we did with the `httr` query in the previous slide but using `arc_select` in `arcgislayers`
...
- Here we pass a similar `arc_select` query as last time for but apply a spatial filter of for waterbodies in Oregon
...
-   Here we usethe `AOI` package for convenience georeferencing to get the spatial boundary of Benton County in Oregon to use as a spatial filter
...
- Rather than downloading GB worth of data locally we can collect data from large rasters via spatial, temporal and attribute queries
